...
public:
    static long int lineSize, lines_per_thread, total_lines, lines_fit;
    static vector<vector<string>> recordsVec;
    static vector<pair<string,int>> metaVec;
...
//    priority_queue<vector<string>,vector<vector<string>>,T> PQ<T>;
    ...
    TwoPhaseMergeSort(string inFile, string outFile, int mem_size, int nthreads, string ord, vector<string> &sort_cols_vec) {
        inputFile = inFile;
        outputFile = outFile;
...
//        exit(0);
    ...
    static void* readInputFile(void *args) {
        ...
        string line="";
        while(cur_lineno<total_lines && cur_lineno<lines_fit+block_num &&  getline(datafile, line)) {
...
//                completed_arr[filenum] = true;
    ...
    static vector<string> recToVec(string line) {
        int start=0;
...
//    cout << endl;
    TwoPhaseMergeSort *tpms = new TwoPhaseMergeSort(argv[1],argv[2],stoi(argv[3]),stoi(argv[4]),argv[5],sort_cols_vec);
